...
const AddressForm: FC<AddressFormProps> = ({ selectdAddress }) => {
  ...
  const validatePostalCode = (v: string): ValidateResult => {
    ...
    return t('noumena.wallet.zipcode.field.error.invalid');
  };

  function containsValidString(street: string) {
    if (PO_BOX_REGEX.test(street)) {
      return t('noumena.wallet.street.field.po.box.error');
    }
    return true;
  }
    ...
    <Stack fullWidth vertical gap={16} data-testid="address-form">
        ...
        {...register('street', {
          ...
          validate: {
            isNotIncludesSpecialCharacters: (v) =>
              ...
              t('noumena.wallet.field.error.special.characters'),
            containsValidString: (v) => containsValidString(v),
          },
        })}
      ...
      <TextField
        {...register('city', {
          ...
          validate: {
            isValidText: (v) =>
              VALID_TEXT_REGEX.test(v) ||
              t('noumena.wallet.field.error.invalid.input'),
          },
        })}
          ...
          {...register('state', {
            ...
            validate: {
              isValidAddress: (v) =>
                ...
                t('noumena.wallet.state.field.error.2.latter.abbreviation'),
            },
          })}
