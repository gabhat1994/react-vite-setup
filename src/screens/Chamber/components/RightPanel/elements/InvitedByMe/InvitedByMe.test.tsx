...
import InvitedByMe from './InvitedByMe';
import { type IInvitedByME, type InvitedByMEProps } from './types';

describe('<InvitedByMe />', () => {
  const user = {
    _id: 'someId',
    access: true,
  };
  ...
  const setup = (props: InvitedByMEProps, mocks: MockedResponse[]) =>
    render(
  ...
  test('renders spinner when loading data ', () => {
    ...
    expect(getByTestId('invited-by-me-skeleton')).toBeInTheDocument();
  });

  test('renders "Invited people who have not yet responded will appear here." when there are no invites fetched from API', async () => {
    const mocks = [createMockedReceivedConnectionRequestResponse([])];
    const props = { noumId: 'testId' };
    ...
    expect(
      getByText('Invited people who have not yet responded will appear here.'),
    ).toBeInTheDocument();
  });
  ...
  test('show max 3 invites when there are more than 3 invites fetched from API', async () => {
