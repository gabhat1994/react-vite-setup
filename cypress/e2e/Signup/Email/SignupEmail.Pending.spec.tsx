...
describe('SignupEmail.Pending.spec.tsx', () => {
  it('Pending Flow Page reload and relogin', () => {
    ...
    SignupPage.moreInfoHeader.should('be.visible');

    // verify reload for pending user
    cy.reload();
    ...
    // verify relogin for pending user
    ...
    HomePage.mainHeaderAvatar
      ...
      .and('have.text', `${firstname} ${lastName}`);
    cy.xpath(
      '//div[@data-test="StyledCompleteInfoCard"]//button[@data-testid="button" and (contains(.,"Noum Me") or contains(.,"Complete your Profile")) and @disabled]',
    ).should('be.visible');
  });
});
